	...
	public ResponseEntity<?> signUp(SignUpRequest request) {
		...
		try {
			if (!(request.getPassword().equals(request.getConfirmPassword()))) {
				...
				response = new ResponseEntity<ServiceResponse>(serviceResponse, HttpStatus.BAD_REQUEST);
				return response;
				...
				if (!(userRole == null)) {
					...
					response = new ResponseEntity<ServiceResponse>(serviceResponse, HttpStatus.UNAUTHORIZED);
					return response;
				}

				serviceResponse.setCode("200");
				serviceResponse.setMessage(request.getUsername() + " you are successfully registered");
				response = new ResponseEntity<ServiceResponse>(serviceResponse, HttpStatus.BAD_REQUEST);
				return response;
			}
